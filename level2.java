...
public class level2  extends JPanel implements ActionListener, ChangeListener  {
	double x = 0, y = 744, Xvel = 1,Yvel = -1,  a = 10;
	double LineX = 110, LineY = 680;
	boolean Collusion = false;
	boolean Alien1Int = false;
	boolean Alien1Int2 = false;
	boolean Alien1IntCheck = false;
	boolean Alien1IntCheck2 = false;
	boolean Visible = false;
	JButton NextLevel2 = new JButton("Next Level");
	Timer t;
	...
	public level2(){
		...
		NextLevel2.setBounds( 550, 300, 150, 50);
	...
	public void paintComponent(Graphics g){
		...
		if(Alien1IntCheck == true && Alien1IntCheck2 == true){
				Visible = true;
		...
		Rectangle2D barrierUpper = new Rectangle2D.Double(1300,450,200,30);
		Rectangle2D ground = new Rectangle2D.Double( 0, 770, 1500, 15);
		Rectangle2D Allien1 = new Rectangle2D.Double( 1240, 690, 40, 80);
		Rectangle2D Allien2 = new Rectangle2D.Double( 1360, 370, 40, 80);
		...
		alien2 = new ImageIcon("alienImage.png");
		if(Alien1Int2 == true){
			Alien1IntCheck2 = true;
		}
		if(Alien1IntCheck2 == false){
			
			alien2.paintIcon(this, g, 1320, 330);
		}
		if(Alien1Int == true){
			Alien1IntCheck = true;
		}
		if(Alien1IntCheck == false){
			...
			alien1.paintIcon(this, g, 1200, 650);
		}
		g2.draw(line);
		g2.draw(barrier);
		g2.draw(barrierUpper);
		g2.draw(Allien1);
		g2.draw(Allien2);
		g2.draw(ball);
		g2.draw(ground);
		...
		Alien1Int2 = ball.intersects(Allien2);
